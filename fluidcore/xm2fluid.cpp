...
int main(int argc, char *argv[]){
	...
	const unsigned notetab[97] = { 0,
		0x20, 0x22, 0x40, 0x41, 0x80, 0xfe, 0xff, 0x100, 0x196, 0x1AF, 0x1C8, 0x1E3,
	...
	unsigned char instr1[257], instr2[257], instr3[257], instr4[257], rspeed[257], instruments[257];
	instruments[0] = 0;
	...
	for (i = 0; i <= (uniqueptns)-1; i++) {
		...
		if (isPatternUsed(i)) {
			...
			for (rows = 1; rows <= ptnlengths[i]; rows++) {
				...
				OUTFILE << "\tdw #" << hex << +rspeed[rows] << "00,#" << +notech1 << ",#" << +notech2;
				OUTFILE << ",(HIGH(smp" << +instr1[rows] << "))*256+(HIGH(smp" << +instr2[rows] << ")),#";
				OUTFILE << +notech3 << ",#" << +notech4 << ",(HIGH(smp" << +instr3[rows] << "))*256+(HIGH(smp" << +instr4[rows] << "))\n";
				
				//update instrument list
				insused = false;
				for (x = 0; x < 256; x++) {
					if (instr1[rows] == instruments[x]) insused = true;
				}
				if (insused == false) {
					insamnt++;
				...
				insused = false;
				for (x = 0; x < 256; x++) {
					if (instr2[rows] == instruments[x]) insused = true;
				}
				if (insused == false) {
					insamnt++;
				...
				insused = false;
				for (x = 0; x < 256; x++) {
					if (instr3[rows] == instruments[x]) insused = true;
				}
				if (insused == false) {
					insamnt++;
				...
				insused = false;
				for (x = 0; x < 256; x++) {
					if (instr4[rows] == instruments[x]) insused = true;
				}
				if (insused == false) {
					insamnt++;
	...
	OUTFILE << "\torg 256*(1+(HIGH($)))\t\t\t;align to 256b page\nsmp0\t\t\t\t;silence\n\tds 256,0\n\n";
