...
print "XM 2 OCTODE 2k15 CONVERTER\n";
...
my (@ch1, @ch2, @ch3, @ch4, @ch5, @ch6, @ch7, @ch8, @drums);
my ($rows, $cpval, $temp, $temp2, $mx, $jx, $nx, $modlen, $noteval, $flags);
my $detune1 = 0;
my $detune2 = 0;
my $detune3 = 0;
my $detune4 = 0;
...
for ($ix = 0; $ix <= ($uniqueptns)-1; $ix++) {
	...
	if ($ptnusage == 1) {
		...
		for ($rows = 1; $rows <= $ptnlengths[($ix)]; $rows++) {	#Achtung! Row values offset by -1 so we can preload dummy values
			...
			for ($mx = 0; $mx <=9; $mx++) {				#reading 10 tracks per row
				...
				if ($cpval >= 128) {				#if we have compressed data
					...
					if ($cpval != 128) {
						...
						if (($cpval&1) == 1) {				#if bit 0 is set, it's note -> counter val.
							...
							$ch7[$rows] = $temp if ($mx == 6);
							$ch8[$rows] = $temp if ($mx == 7);
							$fileoffset++;
						...
						if (($cpval&2) == 2) {				#if bit 1 is set, it's instrument
							$drums[$rows] = 1 if ($temp == 2);
							$drums[$rows] = 5 if ($temp == 3);
							$drums[$rows] = 0x81 if ($temp == 4);
						...
						if (($cpval&8) == 8 && $temp == 14) {				#if bit 3 is set and value is $e
							...
							if (($cpval&16) == 16 && ($temp&0xf0) == 0x50) {	#if upper nibble = 5, it's detune
								...
								$detune7 = int($ch7[$rows]*$temp/100) if ($mx == 6);
								$ch7[$rows] = $ch7[$rows] - $detune7 if ($mx == 6);	#setting detune if bit 4 is set 
								
								$detune8 = int($ch8[$rows]*$temp/100) if ($mx == 7);
								$ch8[$rows] = $ch8[$rows] - $detune8 if ($mx == 7);
					...
					$drums[$rows] = 1 if ($temp == 2);		#set drum if instrument != 0
					$drums[$rows] = 5 if ($temp == 3);
					$drums[$rows] = 0x81 if ($temp == 4);
			...
			$detune6 = 0;
			$ch7[$rows] = $ch7[$rows] + $detune7;
			$detune7 = 0;
			$ch8[$rows] = $ch8[$rows] + $detune8;
			$detune8 = 0;
		}
